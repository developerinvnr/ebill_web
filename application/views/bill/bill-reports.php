  <section class="content mt-2">
          	...
          	<div class="col-sm-12">
        		...
        		<div class="col-sm-2">
        			...
        			<select id="status" class="form-control">
        				...
        				<option value="over_due" <?=isset($status)?($status == 'over_due'?'selected':''):'';?>>OVER DUE</option>
        				<option value="urgent" <?=isset($status)?($status == 'urgent'?'selected':''):'';?>>TODAY</option>
        		...
        		<div class="col mt-4">
          		...
          		<table class="table table-bordered table-sm text-sm border" id="bill_report">
    ...
    function bill_report(){
    	$.ajax({
            url: `${baseUrl}Meter_ctrl/bill_report_ajax`,
            method: "POST",
            dataType: "json",
            data : {
            	'u_id'	: $('#users').val(),
            ...
            success(response){
            	...
            	$('#bill_report').dataTable().fnClearTable();
    			$('#bill_report').dataTable().fnDestroy();
            	if(response.status == 200){
                	var x = '';
                	$.each(response.data,function(key,value){
                	...
                	$('#bill_report_list').html(x);
            	} else {
            		$('#bill_report_list').html('NO record found');
            	}
            	
                $('#bill_report').DataTable({
                       	"searching": true,
                //         "bPaginate": false,
                        "bLengthChange": false,
                        "bFilter": true,
